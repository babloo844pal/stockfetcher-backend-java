...
import com.stockfetcher.cache.InstitutionalHolderRedisService;
import com.stockfetcher.model.InstitutionalHolderEntity;
import com.stockfetcher.model.MetaInfoEntity;
import com.stockfetcher.repository.InstitutionalHolderRepository;

@Service
public class InstitutionalHolderService {

	private final InstitutionalHolderRepository holderRepository;
	private final InstitutionalHolderRedisService redisService;
	...
	public InstitutionalHolderService(InstitutionalHolderRepository holderRepository,
			InstitutionalHolderRedisService redisService, MetaInfoService metaInfoService,
	...
	public List<InstitutionalHolderEntity> getBySymbol(String symbol) {
		String cacheKey = redisService.getCacheKeyBySymbol(symbol);

		// 1. Check in Redis cache
		List<InstitutionalHolderEntity> cachedData = redisService.getFromCache(cacheKey);
		if (cachedData != null)
			return cachedData;

		// 2. Check in Database
		List<InstitutionalHolderEntity> dbData = holderRepository.findByMetaInfo_Symbol(symbol);
		if (!dbData.isEmpty()) {
	...
	private List<InstitutionalHolderEntity> fetchAndSaveFromApi(String symbol) {
		...
		String url = UriComponentsBuilder.fromUriString(API_URL + "/institutional_holders").queryParam("symbol", symbol)
		...
		try {
			...
			List<InstitutionalHolderEntity> holders = new ArrayList<>();
			for (JsonNode node : rootNode.path("institutional_holders")) {
				InstitutionalHolderEntity holder = new InstitutionalHolderEntity();
				holder.setMetaInfo(savedMeta);
			...
			List<InstitutionalHolderEntity> savedHolders = holderRepository.saveAll(holders);
			redisService.saveToCache(redisService.getCacheKeyBySymbol(symbol), savedHolders);
			return savedHolders;

		} catch (Exception e) {
			throw new RuntimeException("Error fetching institutional holders from API", e);
		}
	}
}
