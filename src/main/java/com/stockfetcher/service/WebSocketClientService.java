 ...
 * 
 * @Service public class WebSocketClientService extends AbstractWebSocketHandler
 * {
 * 
 * private static final String WEBSOCKET_URL =
 * "wss://ws-api.iexcloud.io/stable/"; private static final String TOKEN =
 * "YOUR_IEX_CLOUD_API_KEY"; // Replace with your IEX Cloud API key private
 * static final String SUBSCRIBE_MESSAGE_TEMPLATE =
 * "{\"token\": \"%s\", \"event\": \"subscribe\", \"symbols\": \"%s\"}";
 * 
 ...
 * Executors.newSingleThreadExecutor().execute(() -> { StandardWebSocketClient
 * client = new StandardWebSocketClient(); client.doHandshake(this,
 * WEBSOCKET_URL); }); }
 * 
 * @Override public void
 * afterConnectionEstablished(org.springframework.web.socket.WebSocketSession
 * session) throws Exception { // Subscribe to stock symbols (e.g., AAPL, TSLA)
 * String subscribeMessage = String.format(SUBSCRIBE_MESSAGE_TEMPLATE, TOKEN,
 * "AAPL,TSLA,GOOG"); session.sendMessage(new TextMessage(subscribeMessage));
 ...
 * TextMessage message) { System.out.println("Received message: " +
 * message.getPayload());
 * 
