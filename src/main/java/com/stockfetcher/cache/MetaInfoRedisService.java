...
public class MetaInfoRedisService {
	...
	public void saveToCache(String key, MetaInfo data) {
		try {
			redisTemplate.opsForValue().set(key, objectMapper.writeValueAsString(data), CACHE_TIMEOUT,
					TimeUnit.MINUTES);
	...
	public MetaInfo getFromCache(String key) {
		String json = (String) redisTemplate.opsForValue().get(key);
		if (json != null) {
			try {
				return objectMapper.readValue(json, MetaInfo.class);
			} catch (JsonProcessingException e) {
